...
Parte 3
```{r}
datos %>% filter(INC_VALOR_MN>500) %>% filter(INC_VALOR_MN<50000)%>%group_by(ADUA_ADUA_DESCC) %>% summarise(conteo=n()) %>% ggplot(aes(x=fct_reorder(ADUA_ADUA_DESCC,conteo),y=conteo))+geom_col()
```
...
```{r}
datos %>% filter(INC_VALOR_MN>100000000) %>% group_by(ADUA_ADUA_DESCC) %>% summarise(conteo=n())
```
