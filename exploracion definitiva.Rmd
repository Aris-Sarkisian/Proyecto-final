...
grafico<-datos %>% mutate(INC_CANTIDAD=round(as.numeric(INC_CANTIDAD),0),INC_VALOR_MN=round(as.numeric(INC_VALOR_MN),0))%>% filter(INC_VALOR_MN<atipicovalor)%>% filter(INC_CANTIDAD<atipicocantidad)%>% ggplot(aes(x=INC_VALOR_MN,y=INC_CANTIDAD))+geom_hex()
ggplotly(grafico)
```
...
```{r}
tabla2 <- subset(data.frame( table(datos$PAIS_PAIS_DESCC_PAIS_PROCEDENC)),Freq>1000)
aduanas <- subset(datos, PAIS_PAIS_DESCC_PAIS_PROCEDENC %in% tabla2$Var1)
tabla2<- data.frame( with(aduanas, table(ADUA_ADUA_DESCC, PAIS_PAIS_DESCC_PAIS_PROCEDENC) ) )
ggplot(tabla2) + geom_tile(aes(x=ADUA_ADUA_DESCC, y=PAIS_PAIS_DESCC_PAIS_PROCEDENC, fill=Freq))+ theme(axis.text.x = element_text(angle = 90, vjust=0.5, size = 8))+scale_fill_gradient(low="black",high="white")
```
...
```{r}
datos %>% group_by(PAIS_PAIS_DESCC_PAIS_PROCEDENC) %>% summarise(conteo=n()) %>% arrange(desc(conteo))
```

```{r}
datos %>% group_by(ADUA_ADUA_DESCC) %>% summarise(conteo=n()) %>% mutate(proporcion=conteo/sum(conteo))
```
```{r}
atipicovalor<-quantile(as.numeric(datos$INC_VALOR_MN),probs=0.75)+1.5*(quantile(as.numeric(datos$INC_VALOR_MN),probs=0.75)-quantile(as.numeric(datos$INC_VALOR_MN),probs=0.25))
